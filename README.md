...
* [`src/nwbvt/echo/`](src/nwbvt/echo/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/nwbvt/echo/core.cljs): contains the SPA entry point, `init`
* [`test/nwbvt/echo/`](test/nwbvt/echo/): test files (ClojureScript,
...
5. Clone this repo and open a terminal in the `nwbvt.echo` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/nwbvt/echo/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns nwbvt.echo.example
  (:require [nwbvt.echo.config :as config])
